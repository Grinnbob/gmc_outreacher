...
class LoginAction {
    ...
    async get_account() {
        let account = await models.Accounts.findOne({
            _id: this.credentials_id,
        })
        if (account == null) {
            throw MyExceptions.LoginActionError(
                "Account with credentials_id: " +
                    this.credentials_id +
                    " not exists."
            )
        }
    ...
    async login_with_email() {
        ...
        if (!account.login || !account.password) {
            throw MyExceptions.LoginActionError(
                "Empty login or password in account."
            )
        }
    ...
    async is_logged() {
        ...
        if (current_url.includes(links.START_PAGE_SHORTLINK)) {
            ...
            // BAN here
            log.debug(
                "LoginAction: Not logged - BAN here. current url: ",
                current_url
            )
