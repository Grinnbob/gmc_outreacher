...
class Action {
    ...
    async startBrowser() {
        ...
        if (
            ...
            log.error(
                "action: Empty or invalid cookies for credentials_id:",
                this.credentials_id
            )
        }
    ...
    check_block(url) {
        ...
        if (
            url.includes(links.BAN_LINK) ||
            url.includes(links.CHALLENGE_LINK)
        ) {
        ...
        try {
            ...
            if (!current_url.includes(short_url)) {
                ...
                if (current_url.includes("guest_login_sales_nav")) {
                    log.debug(
                        "gotoChecker - Sales Navigator unreachable: ",
                        current_url
                    )
                    throw MyExceptions.SN_access_error(
                        "gotoChecker - Sales Navigator unreachable: " +
                            current_url
                    )
                    ...
                    let loginAction = new LoginAction.LoginAction(
                        this.credentials_id
                    )
                    ...
                    throw new Error(
                        "gotoChecker - We cann't go to page, we got: " +
                            current_url
                    )
                }
            }
