    ...
    try {
        switch (data.action_key) {
            ...
            case "linkedin-post-parsing":
                await workers.post_engagement_worker(data.task_id)
                break
        ...
        await models_shared.TaskQueue.findOneAndUpdate(
            { _id: data.task_id },
            { status: -1, result_data: err_result },
            function (err, res) {
                if (err)
                    throw MyExceptions.MongoDBError("MongoDB save err: " + err)
                // updated!
            }
        )
    }
...
async function taskStatusListener() {
    ...
    cron.schedule("* * * * *", async () => {
        try {
            handler_lock = await models.CronLock.findOneAndUpdate(
                { lock: "cron_lock", ack: 0 },
                { ack: 1 },
                { upsert: true, new: true }
            )

            let tasks = await models_shared.TaskQueue.find(
                {
                ...
                for (let task of tasks) {
                    ...
                    await models_shared.TaskQueue.findOneAndUpdate(
                        ...
                        function (err, res) {
                            if (err)
                                throw MyExceptions.MongoDBError(
                                    "MongoDB updateOne TASK err: " + err
                                )
                        }
                    )

                    log.error(
                        "taskStatusListener: task executed, status: " +
                            task.status +
                            " action_key: " +
                            task.action_key
                    ) // test
            ...
            log.debug(
                "taskStatusListener: ....this message logs every minute - CRON is active...."
            )
        } catch (err) {
            log.debug("taskStatusListener: ....CRON error:....", err)
            log.debug(
                "taskStatusListener: ....CRON error handler_lock:....",
                handler_lock
            )
        } finally {
            log.debug(
                "taskStatusListener: ....CRON finally handler_lock:....",
                handler_lock
            )
            if (handler_lock != null) {
                await models.CronLock.findOneAndUpdate(
                    { lock: "cron_lock" },
                    { ack: 0 },
                    { upsert: false }
                )
